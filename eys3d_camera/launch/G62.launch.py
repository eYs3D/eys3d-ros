#!/usr/bin/env python3
"""Launch shortcut for the eYs3D G62 depth camera.

Equivalent to:
    ros2 launch eys3d_camera camera.launch.py model:=G62 camera_name:=G62_1

The common camera.launch.py arguments are forwarded; `diagnostics_rate_hz`
is available only on `camera.launch.py`.

The default mode (mode_id=1) is L'+D 640x480 at 25 fps. Refer to
launch/video_modes/G62.yaml for the full mode catalogue.
...
def generate_launch_description():
    args = [
        ...
        DeclareLaunchArgument('mode_id',           default_value='1',
                              description='Mode index from launch/video_modes/G62.yaml.'),
        DeclareLaunchArgument('camera_name',       default_value='G62_1',
        ...
        DeclareLaunchArgument('ir_intensity',      default_value='-1',
                              description='-1 = use per-model default (60). '
                                          '0 = projector off. 1-96 = raw FW level.'),
        DeclareLaunchArgument('depth_minimum_mm',  default_value='-1',
                              description='-1 = use default (100 mm); positive integer to override.'),
